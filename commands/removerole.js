...
module.exports.run = async (bot, message, args) => {
  ...
  try{
    await rMember.send(`<@${rMember.id}> has lost the role"${gRole.name}"!`)
  }catch(e){
    message.channel.send(`<@${rMember.id}> has lost the role ${gRole.name}. We tried to DM them, but their DMs are locked!`)
  }
}
